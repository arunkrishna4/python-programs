...
class Qspiders:
    ...
    mbl = 'Basavanagudi'
    subject = 'Testing'
    def __init__(self, name, pno, email, add, gender):
        self.name = name
        self.pno = pno
        self.email = email
        self.add = add
    ...
    def display(self):
        d = self.__dict__
        for attribute, value in d.items():
            print(f"The {attribute} of the Student is {value}")

class Pyspiders(Qspiders):
    mbl = 'National Collage'
    subject = 'Python'
    def __init__(self, name, pno, email, add, gender):
        super().__init__(name, pno, email, add, gender)
        
class Jspiders(Qspiders):
    mbl = 'BTM Layout'
...
s1 = Qspiders('steve', 8529637410, 'steve@gmail.com', 'bbsr', 'Shemale')
s2 = Pyspiders('arun', 7896541230, 'Arun@gmail.com', 'Puri', 'Male')
s3 = Jspiders('chapri', 8855229966, 'Chapri@ktm.com', 'Road', 'Not Defined')
